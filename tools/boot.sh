...
# Raw from Syzkaller:
qemu-system-x86_64 -m 4096 -smp 2 -display none -serial stdio -no-reboot -enable-kvm -cpu host,migratable=off -net nic,model=e1000 -net user,host=10.0.2.10,hostfwd=tcp::1569-:22 -hda $image -snapshot -kernel $kernel -append "$crashargs"

# qemu-system-x86_64 -kernel $kernel -append "$crashargs" -drive file=$image,format=raw -net user,hostfwd=tcp::12000-:22 -net nic,model=e1000 -enable-kvm -nographic -m 2G -smp 2 -pidfile vm.pid -no-reboot -snapshot 2>&1
