...
sudo rm -rf wheezy
mkdir -p wheezy
sudo debootstrap --no-check-gpg --include=openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default wheezy wheezy http://archive.debian.org/debian-archive/debian
